...
**DO NOT edit this file** unless you are currently working in the `zero-friction` repository itself. This is the source of truth that defines how all projects work. Changes made here propagate to every repo.
...
**This file governs all others. Treat it as read-only unless you're in its home repo.**
...
**Note:** Roadmap uses two-level version shorthand (v0.1, v0.2, v1.0) to define milestones. Features within those milestones get three-level versions (v0.1.0, v0.1.1, v0.2.0).
...
**All projects use three-level semantic versioning: `MAJOR.MINOR.PATCH`**

### The Scale
| Version | Meaning |
|---|---|
| `v0.1.0` | First working prototype — something runs |
| `v0.2.0` – `v0.9.0` | Iterative development, MINOR bumps for meaningful milestones |
| `v0.x.1`, `v0.x.2` | PATCH increments for features/fixes within a milestone |
| `v1.0.0` | **MVP — minimum viable product, shippable to real users** |
| `v1.1.0`, `v1.2.0` | Post-MVP improvements and additions |
| `v2.0.0` | Major rewrite, architectural overhaul, or direction change |

### How Roadmap and Features Use Versions

**Roadmap (`01_roadmap.md`) tracks MINOR milestones:**
- Uses two-level shorthand: `v0.1`, `v0.2`, `v1.0`
- Each milestone represents a meaningful, shippable increment
- Example: "v0.2 — GitHub Integration" groups related features

**Features (`02_features.md`) use full three-level versions:**
- Uses complete semver: `v0.1.0`, `v0.1.1`, `v0.2.0`
- Each feature gets a specific PATCH within its milestone
- PATCH increments (0.1.0 → 0.1.1) = features within same milestone
- MINOR increments (0.1.x → 0.2.0) = new milestone started

**Why this matters:**
- Versions must be **sequential and meaningful**
- You cannot jump from v0.1.0 to v0.5.0 without shipping v0.2, v0.3, v0.4 first
- Each increment must represent real, working progress toward v1.0
- When you reach v1.0, the version history should tell a coherent story

### Rules
- **v0.x.y is pre-MVP** — the product is not finished, not promised to anyone
- **v1.0.0 is the milestone you plan toward** — every feature should map to a version ≤ 1.0.0 or beyond
...
- **Versions must progress logically** — no arbitrary jumps, each increment must be earned
- Each `MINOR` increment (0.1.x → 0.2.0) should represent a meaningful, shippable improvement
...
**Roadmap milestones (two-level):**
```
v0.1 — Basic scaffolding works
v0.2 — GitHub integration
v0.3 — Virtual environment automation
v0.4 — DNA propagation
v1.0 — MVP: stable, documented, cross-machine, reliable for daily use
v1.1 — Repo audit tool
v1.2 — Cross-platform (Mac/Linux) support
```

**Feature versioning (three-level):**
```
v0.1.0 — Project creation with git init
v0.1.1 — Add directory structure scaffolding
v0.2.0 — GitHub repo auto-creation (milestone)
v0.2.1 — GitHub branch protection setup
v0.3.0 — Virtual environment setup (milestone)
v0.3.1 — Package dependency installation
v0.3.2 — requirements.txt generation
```
